...
istype函数：测试描述符fd是不是fdtype参数指定的类型
...
若建立连接成功，也就是connect调用成功，状态会再变到ESTABLISHED状态。若函数connect调用失败，则套接字不能再使用，必须关闭。如果想继续想服务器发起建立连接的请求，就需要重新调用sock函数，生成新的套接字
...
......
bzero(&server, sizeof(server));//为套接字
...
![TCP三次握手中两个队列的位置](https://ws3.sinaimg.cn/large/006tNbRwly1fwkopb9ydyj30mo0dmwsm.jpg)

![文字补充](https://ws4.sinaimg.cn/large/006tNbRwly1fwkp1twcqgj30sc0f2x3q.jpg)
